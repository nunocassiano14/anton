                ...
                repeat with terminalTab in tabs of terminalWindow
                    if tty of terminalTab is targetTTY then
                        if requestedAction is "send" then
                            -- Terminal treats text pasted with an embedded
                            -- CR differently from a discrete Return key in
                            -- full-screen CLIs. Send them as two events: the
                            -- reply reaches the original tab, then the second
                            -- event commits it exactly like a user Enter.
                            do script promptText in terminalTab
                            do script (ASCII character 13) in terminalTab
                            return "ok"
                        end if
